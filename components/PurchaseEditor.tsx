...
import ImportFromVendorModal from './ImportFromVendorModal';
...
const PurchaseEditor: React.FC<PurchaseEditorProps> = ({ 
    ...
    const { products, suppliers, integrations } = useAuth();
    ...
    const vendorIntegrations = useMemo(() => integrations.filter(i => i.provider === 'vendor-api'), [integrations]);
    
    const addToCart = useCallback((product: Product, quantity = 1, costPrice?: number) => {
        ...
        setCart(prevCart => {
            ...
            if (existingItem) {
                ...
                if (mode === 'return' && newQuantity > product.stock) {
                ...
                return prevCart.map(item => 
                    item.id === product.id ? { ...item, quantity: newQuantity, price: costPrice ?? item.price } : item
                );
            }
            // When adding, the purchase price defaults to the product's cost price
            setCartErrors(newCartErrors);
            return [...prevCart, { ...product, quantity, price: costPrice ?? product.costPrice }];
        });
    }, [cartErrors, mode]);

    const handleAddProductsFromVendor = (productsToAdd: { productId: string, quantity: number, costPrice: number }[]) => {
        productsToAdd.forEach(({ productId, quantity, costPrice }) => {
            const product = products.find(p => p.id === productId);
            if (product) {
                addToCart(product, quantity, costPrice);
            }
        });
    ...
    return (
        <>
                    ...
                    <div className="p-4 overflow-y-auto">
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            {filteredProducts.map(product => (
                                <div key={product.id} onClick={() => addToCart(product, 1)} className="relative cursor-pointer border dark:border-slate-700 rounded-lg p-2 text-center group transition-all hover:shadow-lg hover:border-indigo-500 hover:scale-105">
                        ...
                        <div className="mt-1 flex gap-2">
                            ...
                            {vendorIntegrations.length > 0 && mode === 'purchase' && (
                                <button
                                    ...
                                    className="flex-shrink-0 px-3 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700"
                                    title="Import from a connected vendor API"
                                >
                                    Import
                                </button>
                            )}
            ...
            {isImportModalOpen && (
                <ImportFromVendorModal
                    onClose={() => setIsImportModalOpen(false)}
                    onAddProducts={handleAddProductsFromVendor}
                />
            )}
        </>
    );
