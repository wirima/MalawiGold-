...
const VendorApiConnectionPage: React.FC = () => {
    const { integrations, addIntegration, updateIntegration, hasPermission } = useAuth();
    const navigate = useNavigate();
    const { connectionId } = useParams<{ connectionId: string }>();

    const isEditing = !!connectionId;
    const existingConnection = isEditing ? integrations.find(i => i.id === connectionId) : null;

    const [name, setName] = useState(existingConnection?.name || '');
    const [apiUrl, setApiUrl] = useState(existingConnection?.config.apiUrl || '');
    const [apiKey, setApiKey] = useState(''); // Always empty for security
    const [isTesting, setIsTesting] = useState(false);
    const [testResult, setTestResult] = useState<'success' | 'failed' | null>(null);
    const [errors, setErrors] = useState({ name: '', apiUrl: '', apiKey: '' });

    useEffect(() => {
        if (isEditing && !existingConnection) {
            // Handle case where ID in URL is invalid
    ...
    const validate = () => {
        const newErrors = { name: '', apiUrl: '', apiKey: '' };
        let isValid = true;
        if (!name.trim()) { newErrors.name = 'Connection name is required.'; isValid = false; }
        if (!apiUrl.trim()) { newErrors.apiUrl = 'API URL is required.'; isValid = false; }
        if (!isEditing && !apiKey.trim()) { newErrors.apiKey = 'API Key is required for new connections.'; isValid = false; }
        setErrors(newErrors);
        return isValid;
    };

    const handleTestConnection = () => {
        if (!apiUrl.trim() || (!isEditing && !apiKey.trim())) {
        ...
        setTimeout(() => {
            ...
            if (apiUrl.includes('http') && (apiKey || isEditing)) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (validate()) {
            const connectionData = {
                provider: 'vendor-api' as const,
                name,
                config: {
                    apiUrl,
                    ...(apiKey && { apiKey }), // Only include apiKey if it has been provided
                },
            };
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md max-w-2xl mx-auto">
            <form onSubmit={handleSubmit} noValidate>
                <div className="p-6 border-b dark:border-slate-700">
                    <h1 className="text-2xl font-bold">{isEditing ? 'Edit Vendor API Connection' : 'Add Vendor API Connection'}</h1>
                    <p className="text-slate-500 dark:text-slate-400 mt-1">Configure the connection to your vendor's API endpoint.</p>
                </div>
                <div className="p-6 space-y-4">
                    <div>
                        <label htmlFor="name" className="block text-sm font-medium">Connection Name*</label>
                        <input type="text" id="name" value={name} onChange={e => setName(e.target.value)} className={`${baseInputClasses} ${errors.name ? errorInputClasses : ''}`} />
                        {errors.name && <p className="mt-1 text-sm text-red-600">{errors.name}</p>}
                    </div>
                     <div>
                        <label htmlFor="apiUrl" className="block text-sm font-medium">API Endpoint URL*</label>
                        <input type="url" id="apiUrl" value={apiUrl} onChange={e => setApiUrl(e.target.value)} placeholder="https://api.vendor.com/v1/products" className={`${baseInputClasses} ${errors.apiUrl ? errorInputClasses : ''}`} />
                         {errors.apiUrl && <p className="mt-1 text-sm text-red-600">{errors.apiUrl}</p>}
                    </div>
                     <div>
                        <label htmlFor="apiKey" className="block text-sm font-medium">API Key*</label>
                        <input type="password" id="apiKey" value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder={isEditing ? 'Leave blank to keep existing key' : 'Enter secret API key'} className={`${baseInputClasses} ${errors.apiKey ? errorInputClasses : ''}`} />
                         {errors.apiKey && <p className="mt-1 text-sm text-red-600">{errors.apiKey}</p>}
                        <p className="mt-1 text-xs text-slate-500">Your API key is securely stored and will not be displayed here again.</p>
                    </div>
                    <div className="pt-2">
                        ...
                        {testResult === 'failed' && <p className="mt-2 text-sm text-red-600">Connection failed. Please check your URL and API key.</p>}
                    </div>
                </div>
...
export default VendorApiConnectionPage;
